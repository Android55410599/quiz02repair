...
public class control599DB extends SQLiteOpenHelper {
	private static final String DATABASE_NAME = "On5mydatabase599";
	...
	public void onCreate(SQLiteDatabase db) {
		...
		db.execSQL("CREATE TABLE "
				+ TABLE_PLAYSTATUS
				+ "(Playerid INTEGER PRIMARY KEY AUTOINCREMENT,Play INTEGER(1), Player CHAR(7), Random INTEGER(1), Position INTEGER(2),Status INTEGER(1),Score INTEGER(2));");
	...
	// Insert PlayStatus
		public long InsertPlayStatus(int strPlay, String strPlayer,int Random, int Position, int Status,int Score) {
	...
	// check PositionScore
		...
		public Integer Random(int random) {
			SQLiteDatabase db;
			db = this.getReadableDatabase();
			String strSQL = "SELECT Random FROM " + TABLE_PLAYSTATUS +" WHERE Random =" + random;
			Cursor cursor = db.rawQuery(strSQL, null);
			if (cursor == null || cursor.getCount() == 0)
				return 0;
			cursor.moveToFirst();
			String position = cursor.getString(cursor.getColumnIndex("Random"));
			int position1 = Integer.parseInt(position);
			return position1;
		}
	...
	public ArrayList<HashMap<String, String>> SelectPlayStatus() {
		...
		try {
			...
			String strSQL = "SELECT Play,Player,Random,Position,Status,Score FROM " + TABLE_PLAYSTATUS;
			...
			if (cursor != null) {
				...
				if (cursor.moveToFirst()) {
					...
					do {
						...
						map.put("Random", cursor.getString(2));
						map.put("Position", cursor.getString(3));
